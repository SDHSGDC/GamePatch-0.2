...
public class PlayerMovementLevelGap : MonoBehaviour {
	...
	void Awake(){
		cameraFollow = GameObject.Find ("Main Camera").GetComponent<CameraFollowLevelGap> ();
	...
	IEnumerator MoveRight() {
		...
		if (cameraFollow.isPushingRight == true) {
			...
			if (cameraFollow.isPushingRight == true && canMoveRight == true && movesLeft > 0) {
	...
	IEnumerator MoveForward() {
		...
		if (cameraFollow.isPushingForward == true) {
			...
			if (cameraFollow.isPushingForward == true && canMoveForward == true && movesLeft > 0) {
